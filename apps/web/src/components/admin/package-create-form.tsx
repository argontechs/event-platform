...
export function PackageCreateForm() {
  ...
  return (
    <form action={action} className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-2">
      ...
      <input name="images" type="file" accept="image/*" multiple className="text-sm text-slate-700 file:mr-3 file:rounded-md file:border-0 file:bg-accent file:px-3 file:py-1.5 file:text-white" />
      <textarea name="description" rows={2} placeholder={t("What's included…")} className={`${field} sm:col-span-2`} />
