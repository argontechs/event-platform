// Shared A4 business document (INVOICE / QUOTATION) — matches the company's
// real Invoice IV-01398 format. Plain black-on-white, prints via .print-document.
...
export function BusinessDocument({
  ...
  const taxable = totals.sstApplied && !totals.b2bExempt;
  ...
  return (
    <div className="mx-auto max-w-[800px] bg-white p-8 text-[12px] text-zinc-900">
      ...
      <table className="mt-4 w-full border-collapse">
        ...
        <tbody>
          {items.map((it, i) => {
            const lineDisc = i === 0 ? totals.discount : 0;
            const lineTax = taxable ? ((it.lineTotal - lineDisc) * totals.sstRate) / 100 : 0;
      ...
      <div className="mt-3 ml-auto w-72 text-[12px]">
        <Row label="Subtotal" value={`RM${money(totals.subtotal)}`} />
        {totals.discount > 0 ? <Row label="Discount" value={`-RM${money(totals.discount)}`} /> : null}
        {totals.sstApplied && totals.b2bExempt ? (
          <Row label="Service Tax" value="Exempt (B2B)" />
        ) : totals.sstApplied ? (
          <Row label={`Service Tax (${totals.sstRate}%)`} value={`RM${money(totals.sstAmount)}`} />
        ) : null}
        {totals.rounding !== 0 ? <Row label="Rounding" value={`RM${money(totals.rounding)}`} /> : null}
        <div className="border-t border-zinc-400" />
        <Row label="Total" value={`RM${money(totals.total)}`} strong />
        {extraRows.map((r, i) => (
      ...
      <div className="mt-6">
        ...
        {remarks ? <p className="whitespace-pre-wrap text-[11px] text-zinc-700">{remarks}</p> : null}
      </div>
